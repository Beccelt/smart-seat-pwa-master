...
void setup() {
  ...
  HM10.begin(9600); //Starts bits per second from the HM-10 to Arduino (baudrate)
...
void initHM10(){ //TODO: Update this method from the loop() stuff up top^
  ...
  if (HM10.available()) {
    c = HM10.read(); //This picked up on the analog readings! NOOOOOOO
    if(c != 0){
      Serial.print(c);
      digitalWrite(LED_PIN2, HIGH);
    }
